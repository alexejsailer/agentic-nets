...
| **Net building** | `deploy_template` · `create_net` · `add_place` · `add_transition` (kind-aware: map/llm/http/command/**agent**/link, pre-wired inscriptions; llm/agent accept a named model `group`) · `set_schedule` · `fire_once` · `start_transition` · `stop_transition` · `create_persona` · **`spawn_persona`** · `scaffold_tool_net` · `invoke_tool_net` · **`crystallize_session`** |
...
| **Observability & debugging** | `net_overview` · `query_tokens` · `event_trail` · `events_wait` · `console_tail` · `model_history` · `transition_history` · `token_lineage` · `failure_context` · `service_logs_tail` · `llm_health` · **`llm_groups`** · **`net_stats`** · **`verify_inscription`** · **`dry_run_transition`** · **`diagnose_transition`** |
