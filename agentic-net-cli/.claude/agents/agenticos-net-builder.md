...
- Agent transitions do NOT consume preset tokens (consume:true is ignored)
...
# PART 6: Agent Role System (rwxh Flags)

| Role | Flag | Tools | Use Case |
|------|------|-------|----------|
| READ_ONLY | `r---` | ~21 tools | Query/inspect only |
| READ_WRITE | `rw--` | ~38 tools | Standard agent work (default) |
| READ_WRITE_EXECUTE | `rwx-` | ~48 tools | + deployment lifecycle |
| FULL | `rwxh` | ~51 tools | + HTTP calls |

**Complete Tool List (53 tools):**

**R (Read) — 21 tools:**
QUERY_TOKENS, LIST_PLACES, GET_PLACE_INFO, GET_PLACE_CONNECTIONS, GET_TRANSITION, VERIFY_RUNTIME_BINDINGS, GET_NET_STRUCTURE, VERIFY_NET, EXPORT_PNML, LIST_ALL_SESSIONS, LIST_ALL_INSCRIPTIONS, LIST_SESSION_NETS, EXTRACT_TOKEN_CONTENT, EXTRACT_RAW_DATA, GET_LINKED_PLACES, FIND_SHARED_PLACES, REGISTRY_LIST_IMAGES, REGISTRY_GET_IMAGE_INFO, PACKAGE_SEARCH, DRY_RUN_TRANSITION, VERIFY_INSCRIPTION, DIAGNOSE_TRANSITION

**W (Write) — 17 tools:**
CREATE_TOKEN, DELETE_TOKEN, CREATE_RUNTIME_PLACE, CREATE_SESSION, CREATE_NET, DELETE_NET, CREATE_PLACE, CREATE_TRANSITION, CREATE_ARC, DELETE_PLACE, DELETE_ARC, SET_INSCRIPTION, NET_DOCTOR, ADAPT_INSCRIPTIONS, EMIT_MEMORY, PACKAGE_PUBLISH, PACKAGE_INSTALL

**X (Execute) — 10 tools:**
DEPLOY_TRANSITION, START_TRANSITION, STOP_TRANSITION, FIRE_ONCE, EXECUTE_TRANSITION, EXECUTE_TRANSITION_SMART, DOCKER_RUN, DOCKER_STOP, DOCKER_LIST, DOCKER_LOGS

**H (HTTP) — 1 tool:**
HTTP_CALL

**Always available — 3 tools:**
THINK, DONE, FAIL
...
**CLI** (`agentic-net-cli`): 73 tools, TypeScript, dual-mode (direct :8080/:8082 or gateway :8083 with JWT).
- Providers: `anthropic`, `claude-code`, `codex`, `ollama`, `routed`
- Build: `cd agentic-net-cli && npx tsup`
- Run: `node dist/bin/agenticos.js`

**Telegram** (`agentic-net-chat`): Inherits CLI tools via shared ToolExecutor. 100 iteration limit, 4h session TTL.

**Tool parity**: CLI and agent transitions share the same tool set (READ_TOOLS, WRITE_TOOLS, EXECUTE_TOOLS). Adding a tool to one should be added to the other.

---

# PART 11: Platform Limitations (Updated 2026-04-16)
...
- **Agent consume:true now reliably consumes** after `MasterEmissionService.java` fix (April 2026). Previously: agents that wrote via CREATE_TOKEN and had empty auto-emit payloads SKIPPED consumption → infinite re-fire. Fix: successful agent actions always consume input tokens.
...
|----------|---------|---------------------|
| **Fabrication** | Invents values when input is missing | `/project/root`, `20240416T...` (wrong year), `STORY-XXX` placeholders |
| **Lossy passthrough** | Garbles long structured data during copy | `testCommand` ends up containing entire Claude Code heredoc, storyId silently empty |
| **Capability over-estimation** | Claims to execute work it can't | PM says "I'll handle this directly" but PM has no tools |
...
Add a MAP after each agent output place. The MAP reads the agent's token + `p-pm-config` as non-consuming preset. It emits a "cleaned" token where missing required fields are filled from config.
...
```json
{
  ...
  "presets": {
    ...
    "config": { "placeId": "p-pm-config", "consume": false, "arcql": "FROM $" }
  },
...
"t-dev-format-cmd": {
  ...
  "presets": {
    ...
    "config": { "placeId": "p-pm-config", "consume": false }
  },
  "action": {
    "template": {
      "args": {
        "command": "...",
        "workingDir": "${config.data.workspaceRoot}"   // ← FORCED, not from task
      }
    }
  }
}
...
The agent that wrote to p-dev-inbox may have set `task.data.workingDir` to garbage (e.g., a relative path like "core/agentic-net-vault"). The map IGNORES that and uses `config.data.workspaceRoot` (an absolute path). The LLM decides WHAT to build; the map decides WHERE commands run.

**When to apply:**
- File paths (use config.workspaceRoot)
- API URLs (use config.masterBaseUrl etc.)
- Credentials (never let agents generate, always read from vault/config)
- Service identifiers (use config)

**Observed real failure this fixes:** An agent put `"core/agentic-net-vault"` as workingDir. Bash ran with CWD=/ so path didn't exist. 68 errors in 7 minutes, retry loop stuck copying the bad value. Fix: force workingDir from config — zero errors, work completed.

## Pattern 5: Config as universal input

Every agent that could invent workingDir/URLs/IDs MUST have `p-pm-config` as a non-consuming preset. Reference `${config.data.workspaceRoot}` explicitly in NL prompts. LLMs follow explicit references better than expecting them to "remember" config.

## Pattern 6 extension: Output Grounding (prevents content fabrication)
...
Verified effective: Agents that previously invented test results now quote real errors like `"Working directory does not exist: core/agentic-net-vault"` directly from stderr.

## Pattern 6: Anti-hallucination prompt rules (put in every agent NL)
...
## Pattern 7: Staging-before-agents (already in use)

Agents can re-fire on the same token. For SHARED inboxes (user-facing places like p-pm-inbox), always put a MAP pass in front that consumes from the shared place into a staging place:
...
| Any agent needing workspace paths | Pattern 5 (config preset) |
| Every agent prompt | Pattern 6 (anti-hallucination rules) |
...
1. **Read before building** — examine existing net structures via `GET_NET_STRUCTURE`, read agent knowledge docs, and consult repo-local net examples
...
7. **Apply LLM mitigation patterns proactively** (Part 12) — any cross-net handoff gets validator+enricher; any user-facing inbox gets a MAP pass; every agent gets anti-hallucination rules and p-pm-config preset
