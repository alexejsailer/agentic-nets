# Changelog — 2026 Q2 archive

Releases `v2.1.2` (2026-04-21) through `v2.16.0` (2026-06-30) of the
AgenticNets open-source services. The active changelog lives at the repo
root [`CHANGELOG.md`](../CHANGELOG.md); see [`README.md`](README.md) for
the rotation rule and the full archive index.
...
### No code changes — released for parity with sibling repo (see [`core/CHANGELOG.md`](../core/CHANGELOG.md) for the actual changes: pre-wired Forge tool-net scaffolds, tool-net concurrency isolation, Forge build-reliability fixes, ghost-run reconciliation, and cross-platform GUI vector icons).
...
### Notes
2.5.1 is a **Telegram bot refactor** in `agentic-net-chat`. The bot now relays user messages to master's persona endpoints (`/api/assistant/p/{persona}/{model}/chat/start` + `/agent-stream` SSE) instead of running its own local agent loop. This matches what the GUI's Universal Assistant does — Telegram and the GUI now share master as the single agent runtime, with identical persona behaviour, tool access, and tier-based model selection. The `core/` side ships a small persona-knowledge-doc fix (see `core/CHANGELOG.md`) so the timestamp examples in the doc no longer get copied verbatim into runtime tokens.

### Changed
- **`agentic-net-chat` — Telegram bridge over master persona SSE** (`agentic-net-chat/src/master/persona-client.ts` (new), `agentic-net-chat/src/chat-state.ts` (new), `agentic-net-chat/src/channel/telegram/telegram-channel.ts` (rewritten), `agentic-net-chat/src/index.ts` (rewritten), `agentic-net-chat/src/session/session-manager.ts` (removed, 615 lines deleted)). The chat container no longer runs a local `ToolExecutor` + `SessionManager` agent loop. On first message in a Telegram chat, it lazily `POST`s `/api/assistant/p/{persona}/{model}/chat/start` through the gateway and receives a `conversationId`. Subsequent messages stream through `/agent-stream` (server-sent events, `fetch().body.getReader()`-based consumer). Text events are forwarded to Telegram; tool-call events are logged as breadcrumbs for ops (not sent to the user — Telegram would flood otherwise). Per-chat state (`persona`, `modelId`, `conversationId`) lives in an in-memory `ChatStateStore`; restarting the container resets the Telegram side, but master keeps its own conversation history regardless. New slash commands: `/personas` (list all personas master exposes), `/persona <id>` (switch persona — resets conversation), `/model <id>` (switch model — resets conversation), `/context` (show current state), `/clear` (start fresh conversation, same persona/model). Defaults: persona = `domain-expert` (the generalist memory layer that auto-bootstraps per-model in 2.5.0), modelId = `process.env['AGENTICOS_MODEL'] ?? 'default'`. The `agentic-net-cli` is unaffected by this refactor — it still runs its own local agent loop and is queued for a parallel refactor in a follow-up release.
- **`agentic-net-chat` — env-var allowlist precedence** (`agentic-net-chat/src/config.ts`). Reads `TELEGRAM_BOT_ALLOWED_CHAT_IDS` first (the canonical name used by `deployment/.env.template` and `docker-compose.yml`), then falls back to legacy `TELEGRAM_ALLOWED_USERS`. Closes a silent bypass observed on 2026-05-18 where the deployment env exported the canonical name but the code only read the legacy name → the allowlist was effectively empty → all users were admitted. Empty/comma-stripped values are treated as "no allowlist" (any Telegram user permitted), matching the prior contract.
- **`agentic-net-cli` — exposes `getAccessToken()` and `getGatewayUrl()` on `GatewayClient`** (`agentic-net-cli/src/gateway/client.ts`). Needed by `agentic-net-chat`'s new `PersonaClient` so it can build its own fetch requests against the SSE endpoint (the existing `masterApi()` helper only does JSON request/response). No CLI behavioural change.
