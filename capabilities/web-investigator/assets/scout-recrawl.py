...
def main():
    ...
    frontier_now = {f.get("canonicalUrl") or canonical_url(f.get("url", "")) for f in rows("p-scout-frontier", 5000)}
    queued, queue_failed = [], 0
    for h in hosts:
        entries = ["https://%s/" % h]
        ...
        for idx, u in enumerate(dict.fromkeys(entries)):
            if canonical_url(u) in frontier_now:
                continue
            try:
                api("POST", "/api/runtime/places/p-scout-frontier/tokens?modelId=" + MODEL,
                    {"name": "recrawl-%s-%d-%s" % (h.replace(".", "-"), idx, ts.replace(":", "")),
                     "data": {"url": u, "canonicalUrl": canonical_url(u), "depth": 0, "attempt": 0,
                              "discoveredFrom": "recrawl", "briefId": brief.get("briefId", ""), "queuedAt": ts}})
                queued.append(u)
            except MasterError as e:
                queue_failed += 1
                _record("queue", "%s %s" % (e.status, e.body))
    out.update(queued=len(queued), queueFailed=queue_failed, hosts=len(hosts), urls=queued[:20],
               outcome="failed" if (queue_failed and not queued) else "ok")
    finish(out)


if __name__ == "__main__":
    main_guard(main, {"kind": "recrawl", "queued": 0, "hosts": 0})
