...
def known_keys(urls):
    """Canonical keys of the given urls that already exist in the registry or the frontier: one
    OR-chain query per place instead of two 3000-row scans. Returns (keys, degraded)."""
    known, degraded = set(), False
...
def main():
    ...
    for u in urls:
        ...
        seen.add(cu)
        host = host_of(u)
        if host in deny or (allow and host not in allow) or not safe_url(u, resolve=False)[0]:
            out["hostFiltered"] += 1
            continue
        cands.append(u)
    known, degraded = known_keys(cands)
    out["dedupeDegraded"] = degraded
    fresh = []
    for u in cands:
        if canonical_url(u) in known or degraded:
            out["known"] += 1
            continue
        fresh.append(u)
    ...
    if fresh:
        ...
        out["frontierCandidates"] = [
            {"url": u, "canonicalUrl": canonical_url(u), "depth": 0, "attempt": 0,
             "discoveredFrom": "search:" + query, "briefId": brief.get("briefId", ""), "queuedAt": now_iso()}
            for u in fresh]
...
if __name__ == "__main__":
    main_guard(main, {"query": "", "provider": "", "found": 0, "queued": 0, "known": 0,
                      "hostFiltered": 0, "malformed": 0, "frontierCandidates": []})
