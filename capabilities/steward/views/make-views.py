#!/usr/bin/env python3
"""Designtime VIEW nets for the Steward.
...
the whole Steward (steward-full) and one small drawing per stage, laid out left to right so each
fits an editor without maximizing. They carry no inscriptions and change nothing at runtime; they
exist so a person can read the Steward one stage at a time. Deleting a view never touches the
runtime (designtime shapes only).

Usage: make-views.py [--model steward] [--session agent-steward] [--write-only]
...
P = lambda s: "p-steward-" + s
T = lambda s: "t-steward-" + s

PLACE_LABELS = {
    "charter": "charter", "coders": "coder definitions", "infra": "infra health", "setup-cmd": "setup commands", "setup-log": "setup log",
    "repo": "pack repository record", "journal": "journal", "errors": "errors", "llm-errors": "contract misses", "observe-cmd": "observe commands",
    "observe-log": "observe log", "health": "health per observation", "lanes": "lane measurements", "map": "the map", "budget": "daily budget",
    "iterate": "iteration triggers", "context-cmd": "brief commands", "loop-log": "loop log", "context": "briefs", "prompts": "questions for the person",
    "responses": "the person's answers", "specs": "specs", "spec-drafts": "drafts for the gate", "gate-cmd": "gate commands", "refused": "refused by the gate",
    "decisions": "the person's decisions", "apply-cmd": "coder commands", "runs": "runs", "verify-cmd": "verify commands", "verification": "verification results",
    "release-cmd": "release commands", "build-log": "build log", "brain-cmd": "brain commands", "brain-log": "brain log", "signals": "signals per release",
    "curation": "curation", "curations": "curation archive", "knowledge": "knowledge facts", "plan": "the plan", "adr": "decision records",
}
TRANSITION_LABELS = {
    "infra-tick": "health tick, hourly", "setup-cmd": "setup process", "observe-cron": "observe, daily", "observe-cmd": "observe, measure",
    "iterate-prep": "trigger to brief", "context-cmd": "render the brief", "propose": "propose, one shot", "answer-prep": "answer to brief",
    "revise-prep": "reshape to brief", "spec": "write the spec, one shot", "gate-prep": "draft to gate", "gate-cmd": "policy gate",
    "approve-prep": "approval to coder", "apply-cmd": "coder, headless", "verify-cmd": "verify, install, roll back", "rollback-prep": "rollback to release",
    "release-cmd": "release, tag, next", "brain-observe-cmd": "observe the release", "curate": "curate, one shot", "curate-cmd": "curate, headless agent",
    "apply-prep": "curation to apply", "answer-knowledge-prep": "answer to brain", "brain-apply-cmd": "apply the curation",
}
VIEWS = [
    ("steward-full", "Designtime view of the Steward: every lane of the four nets in one drawing", None),
    ("steward-view-setup", "Designtime view: stage 0, measure the host, store the token, clone the pack repository", ["infra-tick", "setup-cmd"]),
    ("steward-view-observe", "Designtime view: stage 1, observe the model daily and start an iteration when nothing waits", ["observe-cron", "observe-cmd"]),
    ("steward-view-propose", "Designtime view: stage 2, propose the next step as a choice, an interview or a goal request", ["iterate-prep", "revise-prep", "context-cmd", "propose"]),
    ("steward-view-spec", "Designtime view: stage 3, the person answers, the spec is written, the gate decides or asks", ["answer-prep", "context-cmd", "spec", "gate-prep", "gate-cmd"]),
    ("steward-view-apply", "Designtime view: stage 4, the coder applies the spec, verify installs or rolls back, release tags", ["approve-prep", "apply-cmd", "verify-cmd", "rollback-prep", "release-cmd"]),
    ("steward-view-brain", "Designtime view: the brain, observe, curate, apply, answer", ["brain-observe-cmd", "curate", "curate-cmd", "apply-prep", "brain-apply-cmd", "answer-knowledge-prep"]),
]
...
def build_view(vid, desc, tids, places, transitions, arcs):
    ...
    for pid in sorted(pset):
        net["places"][pid] = {"id": pid, "label": PLACE_LABELS.get(pid[len("p-steward-"):], places[pid]), "tokens": 0, "x": pos[pid][0], "y": pos[pid][1]}
    for tid in sorted(tset):
        net["transitions"][tid] = {"id": tid, "label": TRANSITION_LABELS.get(tid[len("t-steward-"):], transitions[tid]), "x": pos[tid][0], "y": pos[tid][1]}
...
def main():
    ap = argparse.ArgumentParser(); ap.add_argument("--model", default="steward"); ap.add_argument("--session", default="agent-steward"); ap.add_argument("--write-only", action="store_true")
    a = ap.parse_args()
