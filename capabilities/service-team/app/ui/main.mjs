...
const CHARTER_KEYS = ['service', 'goal', 'description', 'repo', 'repoDir', 'scopePaths', 'testCommand', 'buildCommand', 'readinessChecks', 'auditCron', 'coderAgent', 'coderModel', 'coderMaxTurns', 'coderTimeoutMin', 'coderAllowedTools', 'brainAgent', 'autonomyLevel', 'dailyBudgetUsd', 'notes'];
...
class ServiceTeamApp extends HTMLElement {
  constructor() { super(); this.attachShadow({ mode: 'open' }); this._s = {}; this._tab = 'now'; this._form = {}; this._busy = new Set(); this._unsubs = []; }
  ...
  _byId(role, key) { const by = new Map(); for (const r of newest(props(this._s[role]), 'updatedAt').reverse()) if (r[key]) by.set(r[key], r); return [...by.values()]; }
  ...
  _setupTab() {
    ...
    return `<div class="grid">
      <div class="card"><h2>Team ${this._goalUndefined(c) ? '<span class="pill warn">goal not defined</span>' : '<span class="pill ok">goal defined</span>'}</h2>
        ...
        ${this._field('c.repo', 'Workspace repository (core, agentic-nets, ci)', c.repo)}${this._field('c.repoDir', 'Service directory in it', c.repoDir)}${this._field('c.scopePaths', 'Further paths the coder may touch (one per line)', arr(c.scopePaths).join('\n'), 'textarea')}
