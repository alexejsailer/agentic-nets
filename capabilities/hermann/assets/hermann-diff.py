#!/usr/bin/env python3
"""hermann-diff: the review brief, measured from the branch.

usage: hermann-diff.py diff <runId>
Diff stat and a bounded diff of the run's branch against main, the spec, the verification result
and the coder's own summary, rendered as one p-hermann-context token (purpose review) for the
review lane. The reviewer LLM sees this, never the repository.
"""
import json
import os
import sys
...
LANE = "t-hermann-diff-cmd"


def bounded_diff(root, base, head, per_file=140, total=11000):
    rc, names, _ = run(["git", "diff", "--name-only", "%s...%s" % (base, head)], cwd=root, check=False)
    chunks, used = [], 0
    for name in [n for n in names.splitlines() if n.strip()]:
        rc, d, _ = run(["git", "diff", "%s...%s" % (base, head), "--", name], cwd=root, check=False)
        body = "\n".join(d.splitlines()[:per_file])
        if used + len(body) > total:
            chunks.append("... (%s and further files omitted for size)" % name)
            break
        chunks.append(body)
        used += len(body)
    return "\n".join(chunks)
...
def diff(run_id):
    ...
    acceptance = "\n".join("- " + a for a in as_list(s.get("acceptance"))) or "- none"
    brief = "\n".join([
        "# BRIEF FOR HERMANN: review the pull request",
        "runId: %s\nspecId: %s\nnow: %s" % (run_id, s.get("specId", ""), now()),
        "## The spec\n%s: %s\n%s\n\n### Acceptance criteria\n%s\n\n### Out of scope\n%s" % (
            s.get("specId"), s.get("title"), s.get("summary", ""), acceptance, "\n".join("- " + x for x in as_list(s.get("outOfScope"))) or "- none"),
        "## Coder summary\n%s\nNotes: %s\nTests added: %s" % (r.get("coderSummary", ""), r.get("coderNotes", ""), ", ".join(as_list(r.get("coderTestsAdded"))) or "none reported"),
        "## Verification (measured)\nstatus %s; %s tests, %s failed; startup %ss; shutdown %ss; logs structured %s\n%s" % (
            ver.get("status", "?"), ver.get("testsRun", "?"), ver.get("testsFailed", "?"), ver.get("startupSeconds", "?"), ver.get("shutdownSeconds", "?"),
            ver.get("logsStructured", "?"), "\n".join("- " + str(x) for x in as_list(ver.get("evidence")))),
        "## Diff stat\n" + stat.strip(),
        "## Diff (bounded)\n```diff\n%s\n```" % d,
    ])
    data = {"at": now(), "purpose": "review", "runId": run_id, "specId": s.get("specId", ""), "iterationId": s.get("iterationId", ""), "sha": r.get("headSha", ""), "brief": brief[:14000]}
    put_token(P["context"], data, name="ctx-review-%s" % run_id)
    journal(LANE, "diff", "review brief for %s: %s" % (run_id, stat.strip().splitlines()[-1] if stat.strip() else "empty diff"), runId=run_id)
    return {"success": True, "runId": run_id, "chars": len(brief)}


def main(argv):
    if len(argv) < 2 or argv[0] != "diff":
        raise RuntimeError("usage: hermann-diff.py diff <runId>")
    return diff(argv[1])
