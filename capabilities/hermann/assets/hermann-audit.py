...
def f08(dg, root):
    ...
    claims = grep(root, mains, r"SKIP\s+LOCKED|FOR\s+UPDATE|@Lock\(|LockModeType\.PESSIMISTIC", re.I)
    if not scheduled or dep_present(dg, "shedlock") or dep_present(dg, "quartz") or claims:
        score += 1; ev.append("no in-process scheduling, or it is cluster-safe (%d @Scheduled, %s)" % (len(scheduled), ("row claims: " + claims[0].split(":")[0]) if claims else "no scheduling"))
    else:
        ev.append("@Scheduled without a cluster lock or row claims runs once per replica: " + scheduled[0])
